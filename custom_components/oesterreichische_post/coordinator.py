"""Coordinator for the Österreichische Post parcel tracker integration.
...
from .api import OesterreichischePostApiClient, OesterreichischePostApiError
from .const import (
...
class OesterreichischePostCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (OesterreichischePostApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Österreichische Post fetch failed for %s: %s", code, result)
