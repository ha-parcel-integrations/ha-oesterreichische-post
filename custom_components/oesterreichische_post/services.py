"""Services for the Österreichische Post parcel tracker integration.

`oesterreichische_post.track_parcel` / `oesterreichische_post.untrack_parcel` let you add or remove a
...
def _resolve_entry(hass: HomeAssistant):
    """Return the single Österreichische Post hub, or raise when it is not set up."""
    entries = hass.config_entries.async_entries(DOMAIN)
    if not entries:
        raise ServiceValidationError("Österreichische Post is not set up")
    return entries[0]


def async_setup_services(hass: HomeAssistant) -> None:
    """Register the Österreichische Post services (idempotent)."""
    ...
    async def _track(call: ServiceCall) -> None:
        ...
        if not valid_tracking_code(tracking_code):
            raise ServiceValidationError(
                f"'{tracking_code}' is not a valid Österreichische Post tracking code"
            )
        ...
        if (await async_verify_tracking_code(hass, tracking_code)) is False:
            raise ServiceValidationError(
                f"Österreichische Post rejects '{tracking_code}' as a tracking "
                "number"
            )
...
def async_unload_services(hass: HomeAssistant) -> None:
    """Remove the Österreichische Post services (single-entry integration, so on unload)."""
