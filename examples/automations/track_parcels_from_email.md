# Automatic parcel tracking from e-mail (IMAP → Österreichische Post)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Österreichische Post tracking code, and register it with `oesterreichische_post.track_parcel` — fully automatic, no extra custom component required.

Österreichische Post is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `oesterreichische_post.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ oesterreichische_post.track_parcel
                                     │
...
- This integration, with the `oesterreichische_post.track_parcel` action available (field `tracking_code`).
...
Österreichische Post numbers have no letter prefix to anchor on, so the regex matches their two shapes instead:

```
(?<![0-9A-Za-z])(?:[A-Z]{2}[0-9]{9}[A-Z]{2}|[0-9]{12,22})(?![0-9A-Za-z])
```

- the international UPU form — two letters, nine digits, a country code (`RR123456789AT`);
- a bare run of digits.

The digit run is anchored at **12** even though Post accepts numbers from 10 digits: at 10 the pattern starts matching order numbers, invoice numbers and phone numbers in the same mail. If your mails carry shorter numbers, lower the bound and expect more false positives — or leave it and let the AI fallback catch them.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\b[0-9]{12,22}')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you widen it.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@post.at","subject":"Ihre Sendung ist unterwegs",
       "text":"Sendungsnummer: 123456789012345678","initial":true,"folder":"INBOX","username":"test"}'
```

Then `oesterreichische_post.untrack_parcel` the test number afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
