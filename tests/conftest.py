"""pytest configuration for the Österreichische Post test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.oesterreichische_post`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    unmapped status on every poll — but that also makes them leak across tests,
    so whether a warning fires would otherwise depend on test order.
    """
    from custom_components.oesterreichische_post import parcels
