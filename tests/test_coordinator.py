"""Tests for the Österreichische Post coordinator: fetching, caching, events
and the dynamic, status-driven polling cadence.
...
from custom_components.oesterreichische_post.api import OesterreichischePostApiError
...
from custom_components.oesterreichische_post.coordinator import (
    OesterreichischePostCoordinator,
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """The same parcel one hop earlier — still in the network.

    The status comes from the newest event, so moving a parcel back means
    dropping its newest event rather than rewriting a status field.
    """
    shipment = active_shipment(code)
    shipment["status"] = "In transit"
    shipment["sendungsEvents"] = shipment["sendungsEvents"][1:]
    return shipment
...
async def test_polling_stops_when_everything_delivered(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = delivered_shipment()
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_polling_is_hot_for_an_out_for_delivery_parcel(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = active_shipment()  # out for delivery
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_update_merges_multiple_parcels(hass):
    ...
    client.async_get_parcel.side_effect = lambda code: (
        active_shipment() if code == ACTIVE_CODE else delivered_shipment()
    )
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_update_not_found_shows_pending_placeholder(hass):
    ...
    # so an unknown code reads as registered rather than unknown.
    assert data[0]["status"] == ParcelStatus.REGISTERED
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = OesterreichischePostApiError("HTTP 500")
...
async def test_update_backfills_missing_tracking_number(hass):
    ...
    del shipment["activeIdentityCode"]
    del shipment["originalIdentityCode"]
    client = AsyncMock()
    client.async_get_parcel.return_value = shipment
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_update_prunes_cache_for_untracked_parcels(hass):
    ...
    client.async_get_parcel.return_value = delivered_shipment()
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
    coordinator._raw_cache["GONE"] = {"activeIdentityCode": "GONE"}
...
async def test_first_refresh_fires_nothing(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.return_value = active_shipment()
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_delivery_fires_delivered_event_and_not_status_changed(hass):
    ...
    client.async_get_parcel.return_value = active_shipment(ACTIVE_CODE)
    await coordinator._async_update_data()
    client.async_get_parcel.return_value = delivered_shipment(ACTIVE_CODE)
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_no_events_for_parcel_first_seen_delivered(hass):
    ...
    client = AsyncMock()
    client.async_get_parcel.side_effect = lambda code: (
        active_shipment(code) if code == ACTIVE_CODE else delivered_shipment(code)
    )
    coordinator = OesterreichischePostCoordinator(hass, client, entry)
...
async def test_fires_registered_event_for_new_parcel(hass):
    ...
    client.async_get_parcel.side_effect = lambda code: active_shipment(code)
    await coordinator._async_update_data()
    await hass.async_block_till_done()
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    assert len(events) == 1
    assert events[0].data["old_planned_from"] == "2026-04-29T13:00:00+02:00"
    assert events[0].data["new_planned_from"] == "2026-04-29T16:00:00+02:00"
...
async def test_losing_the_eta_is_silent(hass):
    ...
    dropped = active_shipment()
    dropped["estimatedDelivery"] = None
    dropped["estimatedDeliveryDate"] = None
